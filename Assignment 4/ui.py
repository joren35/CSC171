...
class Gui:
    def __init__(self):
        ...
        self.board_img = pygame.image.load(os.path.join(
            "imagefold", "board.bmp")).convert()
        self.black_img = pygame.image.load(os.path.join(
            "imagefold", "black.bmp")).convert()
        self.white_img = pygame.image.load(os.path.join(
            "imagefold", "white.bmp")).convert()
        self.tip_img = pygame.image.load(os.path.join("imagefold",
                                                      "tip.bmp")).convert()
        self.clear_img = pygame.image.load(os.path.join("imagefold",
                                                        "em.bmp")).convert()
    ...
    def show_options(self):
        ...
        while True:
            ...
            title_fnt = pygame.font.SysFont("Arial", 34)
            title = title_fnt.render("CSC 171 Othello", True, self.WHITE)
            title_pos = title.get_rect(
                centerx=self.screen.get_width() / 2, centery=60)

            start_txt = self.font.render("Begin", True, self.WHITE)
            start_pos = start_txt.get_rect(
                centerx=self.screen.get_width() / 2, centery=220)
            player1_txt = self.font.render("Select First Player", True, self.WHITE)
            player1_pos = player1_txt.get_rect(
                centerx=self.screen.get_width() / 2, centery=260)
            player2_txt = self.font.render("Select Second Player", True, self.WHITE)
            player2_pos = player2_txt.get_rect(
                centerx=self.screen.get_width() / 2, centery=300)
            level_txt = self.font.render("Select AI difficulty", True, self.WHITE)
    ...
    def show_winner(self, player_color):
        ...
        while True:
            ...
            again_txt = self.font.render("Click Here to Main Menu", True, self.WHITE)
            again_pos = again_txt.get_rect(
                    centerx=self.screen.get_width() / 2, centery=220)

            self.screen.blit(again_txt, again_pos)
            ...
            for event in pygame.event.get():
                if event.type == QUIT:
                    ...
                    (mouse_x, mouse_y) = pygame.mouse.get_pos()
                    if again_pos.collidepoint(mouse_x, mouse_y):
                        return None
    ...
    def get_chosen_player(self):
        ...
        while True:
            self.screen.fill(self.BACKGROUND)
            title_fnt = pygame.font.SysFont("Arial", 34)
            title = title_fnt.render("Choose Player", True, Color(255, 255, 255))
            title_pos = title.get_rect(
                centerx=self.screen.get_width() / 2, centery=60)
            human_txt = self.font.render("Human", True, self.WHITE)
    ...
    def get_chosen_level(self):
        ...
        while True:
            self.screen.fill(self.BACKGROUND)
            title_fnt = pygame.font.SysFont("Arial", 34)
            title = title_fnt.render("Select AI Difficulty", True, self.WHITE)
            title_pos = title.get_rect(
                centerx=self.screen.get_width() / 2, centery=60)
            one_txt = self.font.render("Easy", True, self.WHITE)
            one_pos = one_txt.get_rect(
                centerx=self.screen.get_width() / 2, centery=160)
            two_txt = self.font.render("Medium", True, self.WHITE)
            two_pos = two_txt.get_rect(
                centerx=self.screen.get_width() / 2, centery=240)

            three_txt = self.font.render("Hard", True, self.WHITE)
    ...
    def showScore(self, blackStr, whiteStr, current_player_color):
        ...
        self.screen.blit(text,
                         (self.BLACK_LAB_POS[0], 30))
        self.screen.blit(text2,
                         (self.WHITE_LAB_POS[0], 30))
